...
public class Vertice extends JPanel implements MouseMotionListener {
    ...
    private void leeClick() {
        String modo = VentanaPrincipal.getModo();
        JPanel mainPanel = VentanaPrincipal.getPanelGrafo();
        Grafo grafo = VentanaPrincipal.getGrafo();
        switch (modo) {
            ...
            case "Recorrido en Amplitud" -> {
                grafo.descoloreaGrafo();
                VentanaPrincipal.getInfoLabel().setText(grafo.recorreEnAmplitud(this));
            }
            case "Recorrido en Profundidad" -> {
                grafo.descoloreaGrafo();
                VentanaPrincipal.getInfoLabel().setText(grafo.recorreEnProfundidad(this));
            }
            case "Algoritmo de Dijkstra" -> {
                grafo.descoloreaGrafo();
                VentanaPrincipal.getInfoLabel().setText(grafo.algoritmoDijkstra(this));
            }
            case "Árbol de expansión mínima" -> {
                grafo.descoloreaGrafo();
                grafo.arbolExpansionMinima(this);
                VentanaPrincipal.getInfoLabel().setText("Click en el panel para borrar aristas grises");
            }
            default -> {}
        }
    }
